...
const { makeShadow, startShadow } = require('./shadowExec')
...
function handleStartInstrumentation () {
  const { backEnd, backEndPort, frontEndPort, targetDir, rootDir, startScript } = readConfig()
  // killPorts()
  // check platform
  // if windows
  // grab pid on port
  ...
  // if linux
  // grab pid on port
  // kill frontEnd and backEnd ports

  // make shadow copy of directory
  console.log('makeShadow')
  makeShadow(rootDir, targetDir)
  ...
  const arrJSFiles = filesController.dirRecursiveContents(targetDir + '/backend/controllers', [], ['.js'])
  ...
  for (const file of arrJSFiles) {
    ...
    const code = instrumentController.transformFile(file)
  ...
  startShadow(targetDir, startScript)
}
